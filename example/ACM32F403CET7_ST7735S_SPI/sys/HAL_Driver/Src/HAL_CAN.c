...
HAL_StatusTypeDef HAL_CAN_Init(CAN_HandleTypeDef *hcan)
{
    ...
    if(hcan->Init.CAN_Mode == CAN_Mode_SelfTest)
        HAL_CAN_OperatingModeRequest(hcan,CAN_OperatingMode_SelfTest);
...
*                         DLC and CAN data.
* Input       : fu32_Timeout: timeout value for can transmiting
* Output      : 
* Author      : CWT                         Data : 2020
**********************************************************************************/
HAL_StatusTypeDef HAL_CAN_Transmit(CAN_HandleTypeDef *hcan, CanTxRxMsg* TxMessage, uint32_t fu32_Timeout)
{
    uint8_t i = 0;
    uint8_t can_id[4];  
    ...
    hcan->Instance->CMR |= (hcan->Init.CAN_Mode == CAN_Mode_Normal) ? CAN_CMR_TR : CAN_CMR_SRR;  // transfer 
		
		if(fu32_Timeout == 0)
		{
        while((hcan->Instance->SR & CAN_SR_TCS)==0x00);
		}
		else
		{
...
* Author      : CWT                         Data : 2020
**********************************************************************************/
HAL_StatusTypeDef HAL_CAN_Receive(CAN_HandleTypeDef *hcan, CanTxRxMsg* RxMessage, uint32_t fu32_Timeout)
{
    uint32_t lu32_Timeout;
    /* Check the parameters */
    if(!IS_CAN_ALL_PERIPH(hcan->Instance)) return HAL_ERROR ;
